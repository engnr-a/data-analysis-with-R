---
title: "Shiny DashBoard by Suleiman"
output: 
...
germany_sp <-readRDS("gadm36_DEU_1_sf.rds")
...
sales_data_filtered2 <- reactive({
  ...
  bike_orderlines_tbl %>%
    ...
    mutate(date_rounded = ymd(lubridate::floor_date(order_date, unit = input$radioB))) %>%
    
    filter(date_rounded %>% between(left  = ymd(input$order_date[1]), right = ymd(input$order_date[2]))) %>%
...
```{r}
# Reactive Filter
output$plot_output2 <- renderPlotly({
   sales_data_filtered2() %>%
